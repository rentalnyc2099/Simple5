...
def generate_document(template, client_data, output_file_path):
    ...
    final_content = template.render(client_data)
...
client_files = [f for f in os.listdir(client_data_dir) if f.endswith(".json")]
print(f"Found {len(client_files)} client files to process.")  # Debugging print
